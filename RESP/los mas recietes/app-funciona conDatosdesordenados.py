...
# PANTALLA 1: ROMPER EL HIELO Y REGISTRO (Carga Instantánea)
...
# ------------------------------------------------------------------
# PANTALLA 2: CHAT CON BLINK (Solo se ejecuta tras el login)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Función con Cache Optimizado (Carga en segundo plano)
# ------------------------------------------------------------------
...
def setup_rag_graph():
    ...
    # Embeddings con HuggingFace en máquina local
    ...
    def generate_node(state: RAGState) -> dict:
        ...
        # Generación formateada localmente usando el contexto del RAG y perfil
        respuesta = (
            f"¡Hola **{user['nombre']}**! Con gusto te informo:\n\n"
            f"De acuerdo con nuestra base de datos e información del catálogo:\n\n"
            f"{context_text}\n\n"
            f"*(Perfil del usuario: {user['email']} | Nivel de tarifa asignado: {user.get('lista_precios', 'Lista 1')})*"
        )
...
# Entrada del Usuario
# ------------------------------------------------------------------
question = st.text_input("Escribe tu mensaje aquí:", key="chat_input")
